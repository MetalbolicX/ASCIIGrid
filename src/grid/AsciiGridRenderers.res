...
let getCorners = (sepType, theme) =>
  switch sepType {
  | Bottom => (theme.AsciiGridTheme.lowerLeft, theme.AsciiGridTheme.lowerRight, theme.AsciiGridTheme.intersectionBottom)
  | TitleBottom | Middle => (theme.AsciiGridTheme.intersectionLeft, theme.AsciiGridTheme.intersectionRight, theme.AsciiGridTheme.intersection)
  | Top | TitleTop => (theme.AsciiGridTheme.upperLeft, theme.AsciiGridTheme.upperRight, theme.AsciiGridTheme.intersectionTop)
  }

let totalInnerWidth = (colWidths, padding) =>
  Belt.Array.reduce(colWidths, 0, (sum, width) => sum + width + padding * 2)

let createSeparator = (colWidths, sepType, theme, padding) => {
  let (leftCorner, rightCorner, between) = getCorners(sepType, theme)
  let len = colWidths->Belt.Array.length

  if sepType == TitleTop {
    let inner = totalInnerWidth(colWidths, padding) + if len > 0 { len - 1 } else { 0 }
    ...
    let rec loop = (i, acc) =>
      if i >= len {
        ...
        let width = switch Belt.Array.get(colWidths, i) {
        | Some(w) => w
        | None => 0
        }
        let segment = acc ++ theme.AsciiGridTheme.line->String.repeat(width + padding * 2)
        let accNext = if i < len - 1 { segment ++ between } else { segment }
        loop(i + 1, accNext)
      }
...
let renderCell = (value: string, colWidth: int, padding: int, align: bool): string => {
  ...
  let safeLen = if targetLen >= 0 { targetLen } else { 0 }

  if align && isNumeric(value) {
    let leftSpaces = safeLen - value->String.length - padding
    let sanitized = if leftSpaces > 0 { leftSpaces } else { 0 }
    " "->String.repeat(sanitized) ++ value ++ pad
  } else {
    let rightSpaces = safeLen - value->String.length - padding
    let sanitized = if rightSpaces > 0 { rightSpaces } else { 0 }
    pad ++ value ++ " "->String.repeat(sanitized)
  }
}

let renderRow = (row: array<string>, colWidths: array<int>, theme: AsciiGridTheme.t, padding: int, align: bool): string => {
  let cells = row->Belt.Array.mapWithIndex((idx, cell) => {
    let width = switch Belt.Array.get(colWidths, idx) {
    | Some(w) => w
    | None => 0
    }
    renderCell(cell, width, padding, align)
  })
  theme.AsciiGridTheme.wall ++ cells->Belt.Array.reduce("", (acc, cell) => if acc == "" { cell } else { acc ++ theme.AsciiGridTheme.wall ++ cell }) ++ theme.AsciiGridTheme.wall
}
